...
import { normalizeWhatsAppLink, getCentreName } from '@/lib/centreUtils'
...
export default function ContactModal({ isOpen, onClose, centre }) {
  ...
  const displayName = getCentreName(centre.name)
  const whatsappLink = normalizeWhatsAppLink(centre.whatsapp_number)
  ...
  const handleWebsite = () => {
    if (centre.website_url) {
      window.open(centre.website_url, '_blank')
    }
  }
